...
Given(
  /^the board is empty except for a Red Guard at \((\d+), (\d+)\)$/,
  function (row: string, col: string) {
    this.board = new Board();
    this.chessService = new ChessService(this.board);
    const guard = new Piece(PieceType.Guard, PlayerColor.Red, {
      row: parseInt(row),
      col: parseInt(col),
    });
    this.board.addPiece(guard);
  },
);

When(
  /^Red moves the Guard from \((\d+), (\d+)\) to \((\d+), (\d+)\)$/,
