...
Given(
  /^the board is empty except for a Red Cannon at \((\d+), (\d+)\)$/,
  function (row: string, col: string) {
    this.board = new Board();
    this.chessService = new ChessService(this.board);
    const cannon = new Piece(PieceType.Cannon, PlayerColor.Red, {
      row: parseInt(row),
      col: parseInt(col),
    });
    this.board.addPiece(cannon);
  },
);

When(
  /^Red moves the Cannon from \((\d+), (\d+)\) to \((\d+), (\d+)\)$/,
