...
Given(
  /^the board is empty except for a Red Elephant at \((\d+), (\d+)\)$/,
  function (row: string, col: string) {
    this.board = new Board();
    this.chessService = new ChessService(this.board);
    const elephant = new Piece(PieceType.Elephant, PlayerColor.Red, {
      row: parseInt(row),
      col: parseInt(col),
    });
    this.board.addPiece(elephant);
  },
);

When(
  /^Red moves the Elephant from \((\d+), (\d+)\) to \((\d+), (\d+)\)$/,
